...
int insert(struct Heap *collection){
...
int insertheapsort(struct Heap *collection){
...
void sort(struct Heap *collection){
int i;
    for(i=1;i<=collection->size;i++)
    {
        cout<<findmin(collection)<<' ';
        Delete(collection);
    }
    cout<<endl;
    return ;
...
struct Heap heapsort;
cin>>heapsort.size;
int i=0;
for(i=1;i<=heapsort.size;i++)
cin>>heapsort.heap[i];

insertheapsort(&heapsort);
